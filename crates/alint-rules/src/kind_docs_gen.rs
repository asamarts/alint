...
pub static KIND_SUMMARIES: &[(&str, &str)] = &[
    ...
    ("git_commit_subject_matches", "Each commit's subject line (the first line of its message) must match the matches: regex, the..."),
    ("git_no_denied_paths", "Fire when any tracked file matches a configured glob denylist."),
    ("import_gate", "Forbid imports whose extracted target matches a forbid regex, within the paths scope, an..."),
    ("indent_style", "Every non-blank line indents with the configured style (tabs or spaces)."),
    ("json_path_equals", "Query a structured document with a JSONPath expression and assert every match deep-equals the..."),
    ...
    ("ordered_block", "The lines between a start / end marker pair must stay sorted (and, with unique: true, free of..."),
    ...
    ("pair_hash", "The algorithm digest (sha256 default / sha512) of every file matching source must appear in the..."),
    ...
    ("toml_path_equals", "Query a structured document with a JSONPath expression and assert every match deep-equals the..."),
    ("toml_path_matches", "Same shape as the *_equals variants, but the asserted value is a regex matched against string..."),
    ("unique_by", "No two files matching select may share the value of key (a path template; tokens..."),
    ("xml_path_equals", "Query a structured document with a JSONPath expression and assert every match deep-equals the..."),
    ("xml_path_matches", "Same shape as the *_equals variants, but the asserted value is a regex matched against string..."),
    ("yaml_path_equals", "Query a structured document with a JSONPath expression and assert every match deep-equals the..."),
