...
- 🧰 **Powerful + extensible.** 105 rule kinds across 13 families, 22 bundled ecosystem rulesets, 24 auto-fix ops, 8 output formats, structured-query rules with full RFC 9535 JSONPath, cross-file relational rules, conditional `when:` gates over per-run facts, and `extends:` composition with SRI-pinned URLs.
...
- **Auto-fix**: 24 ops covering content edits (whitespace, newlines, line endings, BOM/bidi/zero-width strip, blank-line collapse), path/dir/mode/VCS ops (create/remove/rename/prepend/append/chmod/`dir_create`, plus `relocate` to move a stray file such as a nested lockfile back to the repo root and `git_untrack` to drop a committed artifact from the index), located value edits (`set_value`/`remove_value` on structured configs, regex `replace`), cross-file ops (`sync_from` to mirror a drifted file from its canonical source, and `create_and_register` to add an unregistered workspace member to a manifest list such as `workspace.members`), block sorting (`sort` keeps a marked `ordered_block` -- or a whole markerless file such as `CODEOWNERS` -- ordered under its comparator, dropping duplicates when `unique`), reindentation (`indent_style` converts tab-indented lines to spaces for a `style: spaces` + `width:` rule), and a plugin `command` fix (run a user-supplied fix-on-save tool such as `eslint --fix`). Preview with `alint fix --dry-run`. Configurable `fix_size_limit` (default 1 MiB) skips oversize files rather than rewriting them.
