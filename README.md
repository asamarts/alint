...
- 🧰 **Powerful + extensible.** 105 rule kinds across 13 families, 22 bundled ecosystem rulesets, 18 auto-fix ops, 8 output formats, structured-query rules with full RFC 9535 JSONPath, cross-file relational rules, conditional `when:` gates over per-run facts, and `extends:` composition with SRI-pinned URLs.
...
- **Auto-fix**: 18 ops covering content edits (whitespace, newlines, line endings, BOM/bidi/zero-width strip, blank-line collapse), path/mode/VCS ops (create/remove/rename/prepend/append/chmod, plus `git_untrack` to drop a committed artifact from the index), located value edits (`set_value`/`remove_value` on structured configs, regex `replace`), and a plugin `command` fix (run a user-supplied fix-on-save tool such as `eslint --fix`). Preview with `alint fix --dry-run`. Configurable `fix_size_limit` (default 1 MiB) skips oversize files rather than rewriting them.
