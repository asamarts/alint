...
- 🧰 **Powerful + extensible.** 105 rule kinds across 13 families, 22 bundled ecosystem rulesets, 26 auto-fix ops, 8 output formats, structured-query rules with full RFC 9535 JSONPath, cross-file relational rules, conditional `when:` gates over per-run facts, and `extends:` composition with SRI-pinned URLs.
...
- **Auto-fix**: 26 ops covering content edits (whitespace, newlines, line endings, BOM/bidi/zero-width strip, blank-line collapse), path/dir/mode/VCS ops (create/remove/rename/prepend/append/chmod/`dir_create`, plus `relocate` to move a stray file such as a nested lockfile back to the repo root and `git_untrack` to drop a committed artifact from the index), located value edits (`set_value`/`remove_value` on structured configs, regex `replace`), cross-file ops (`sync_from` to mirror a drifted file from its canonical source, and `create_and_register` to add an unregistered workspace member to a manifest list such as `workspace.members`), list maintenance on a marked `ordered_block` or a whole markerless file such as `CODEOWNERS` (`sort` keeps it ordered under its comparator, dropping duplicates when `unique`; `insert_line` splices a `require:`d line into a markerless list at its sorted position, Unsafe by default), header insertion (`insert_header` places a `file_header`'s required header AFTER a leading shebang or XML declaration, rather than above it at BOF as `file_prepend` would), reindentation (`indent_style` converts tab-indented lines to spaces for a `style: spaces` + `width:` rule), and a plugin `command` fix (run a user-supplied fix-on-save tool such as `eslint --fix`). Preview with `alint fix --dry-run`. Configurable `fix_size_limit` (default 1 MiB) skips oversize files rather than rewriting them.
...
- **Official GitHub Action**: [`asamarts/alint@v0.16.1`](https://github.com/asamarts/alint).
...
- uses: asamarts/alint@v0.16.1
  with:
    version: v0.16.1        # alint release tag (default: latest)
