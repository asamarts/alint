...
- **Official GitHub Action** — `asamarts/alint@v0.5.6`.
...
docker run --rm -v "$PWD:/repo" ghcr.io/asamarts/alint:v0.5.6 check
```
...
Also published: `:<major>.<minor>` (e.g. `:0.5`) and the raw git tag (`:v0.5.6`).
...
- uses: asamarts/alint@v0.5.6
  with:
    version: v0.5.6        # alint release tag (default: latest)
