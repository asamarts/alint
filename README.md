...
- **Official GitHub Action** — `asamarts/alint@v0.8.2`.
...
docker run --rm -v "$PWD:/repo" ghcr.io/asamarts/alint:v0.8.2 check
```
...
Also published: `:<major>.<minor>` (e.g. `:0.8`) and the raw git tag (`:v0.8.2`).
...
- uses: asamarts/alint@v0.8.2
  with:
    version: v0.8.2        # alint release tag (default: latest)
