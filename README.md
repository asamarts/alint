...
- **Official GitHub Action** — `asamarts/alint@v0.9.3`.
...
docker run --rm -v "$PWD:/repo" ghcr.io/asamarts/alint:v0.9.3 check
```
...
Also published: `:<major>.<minor>` (e.g. `:0.9`) and the raw git tag (`:v0.9.3`).
...
- uses: asamarts/alint@v0.9.3
  with:
    version: v0.9.3        # alint release tag (default: latest)
