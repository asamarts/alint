...
if (!fs.existsSync(binaryPath)) {
  process.stderr.write(
    `@asamarts/alint: binary not found at ${binaryPath}\n` +
      `  this usually means the postinstall step failed; try:\n` +
      `    npm uninstall -g @asamarts/alint && npm install -g @asamarts/alint\n` +
...
if (result.error) {
  process.stderr.write(`@asamarts/alint: failed to spawn ${binaryPath}: ${result.error.message}\n`);
  process.exit(1);
}
