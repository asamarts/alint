...
**Value-mapping caveats (all formats).** The eight formats coerce into one `serde_json::Value` tree, but their data models differ, so a rule that works on one format can behave differently on another. Five sharp edges:
...
- **`inf` / `nan` map to `null`.** TOML and YAML accept IEEE `inf` / `.inf` / `nan`, but the query tree cannot represent a non-finite float, so they arrive as `null` (an `equals: null` matches; a "must have a value" check passes). HCL rejects them as a parse error; JSON has no such literal.
...
- **Large / based numbers diverge.** An integer beyond `i64` becomes a lossy float in JSON but a parse error in YAML / TOML / HCL. TOML and YAML also parse `0x1F` / `0o17` / `1_000` as numbers where HCL does not, and the stringly formats keep every number as a string.
