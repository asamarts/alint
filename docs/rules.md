...
- **`inf` / `nan` are not numbers you can match normally.** TOML and YAML accept IEEE `inf` / `.inf` / `nan`, but the query tree cannot represent a non-finite float, so they arrive as `null` (an `equals: null` matches; a "must have a value" check passes). HCL does NOT error on them either: it keeps them as the opaque expression strings `${inf}` / `${nan}`. JSON has no such literal.
