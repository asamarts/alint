...
| `https://alint.org/robots.txt` | **stale** | Cloudflare-managed `content-signal` block (`search=yes,ai-train=no`). Explicit Disallow for: ClaudeBot, GPTBot, CCBot, Bytespider, Google-Extended, meta-externalagent, Applebot-Extended, Amazonbot, CloudflareBrowserRenderingCrawler. **Missing PerplexityBot + anthropic-ai (legacy UA).** Also: current posture is **opt-out of AI training** — needs a deliberate decision before P3.3 flips to opt-in. | P3.3 work + a strategic decision: do we want AI-train = no (current default) or yes? |
...
3. **AI-training opt-out posture** — current robots.txt blocks
   ClaudeBot/GPTBot/etc. The launch-prep plan assumed opt-in. Needs an
   explicit user decision before P3.3 changes posture.
