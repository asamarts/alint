...
[`architecture/crate-graph.md`](./architecture/crate-graph.md) — a Mermaid diagram plus a
crate-by-tier table, regenerated by `cargo run -p xtask -- gen-arch` and gated by
`gen-arch --check` (so it can't drift from the Cargo manifests). `alint-core` is the
foundation (a dependency sink, enforced by a test); the `alint` binary and `xtask` sit at the
top.

The **C4 model** (intent: system context, containers, the crates as components) is hand-
modeled in [`architecture/workspace.dsl`](./architecture/workspace.dsl) (Structurizr DSL).
`gen-arch --check` verifies its crate components equal the real workspace members, so the
model can't silently omit a crate. Architecture decisions are recorded as ADRs under
[`../adr/`](../adr/). See [architecture-as-code.md](./architecture-as-code.md) for the
generator design (Phase 4 / WS3).
