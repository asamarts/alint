...
— a Mermaid diagram (solid = runtime deps, dashed = dev/build-only) plus a crate-by-tier
...
modeled in
[`docs/design/architecture/workspace.dsl`](https://github.com/asamarts/alint/blob/main/docs/design/architecture/workspace.dsl)
(Structurizr DSL). `gen-arch --check` verifies its crate components equal the real workspace
members, so the model can't silently omit a crate. Architecture decisions are recorded as ADRs
under [`docs/adr/`](https://github.com/asamarts/alint/tree/main/docs/adr). See
[`architecture-as-code.md`](https://github.com/asamarts/alint/blob/main/docs/design/architecture-as-code.md)
for the generator design (Phase 4 / WS3).
