...
- **Parse-tree memory amplification (per-file cap: FIXED).** `MAX_ANALYZE_BYTES` (256 MiB, `walker.rs`) bounds INPUT bytes, but a structured tree measures ~16-19x the input, so a near-read-cap file peaked at multiple GB of RSS. `Format::parse` now caps structured INPUT at `MAX_STRUCTURED_BYTES` (64 MiB, `structured_format.rs`; HCL keeps its tighter 64 KiB cap), bounding a single file's tree to a few hundred MB (4x under the read cap) while staying far above any real config / manifest. An oversize structured file is one per-file parse-error violation.
