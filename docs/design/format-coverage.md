...
Surfaced by the pre-v0.16 adversarial audit (round 3). None is a crafted-small-file DoS (those are all fixed: HCL/YAML/XML depth guards, the HCL byte cap, and the per-element XML attribute cap); each needs an extreme input AND a resource-constrained host, and each is a GENERAL-engine concern (not specific to the format-coverage arc), so the fixes are deferred to a dedicated robustness pass rather than rushed into this hardening PR:

1. **Parse-tree memory amplification.** `MAX_ANALYZE_BYTES` (256 MiB, `walker.rs`) bounds INPUT bytes, but a structured format expands into a `serde_json::Value` tree ~10-16x the input (XML worst; dotenv ~10x). A near-cap file can peak at multiple GB of RSS, multiplied by the `par_iter` fan-out. Fix options: a smaller structured-parse-specific cap, and/or a bound on total concurrent parse memory.
2. **rayon global-pool init can panic under thread-creation pressure.** Both engine dispatch loops use rayon `par_iter`; if the OS refuses rayon's worker threads (`RLIMIT_NPROC` / `RLIMIT_AS` / `vm.max_map_count` pressure) the lazy global-pool init panics (`rayon-core`), producing the "alint crashed" banner instead of a graceful error. Fix: build a scoped pool with a sequential fallback and `install()` the parallel sections. (The arc's own analogue -- the HCL 512 MB parse thread failing to spawn -- IS fixed: it now returns a per-file parse-error instead of `.expect`-panicking.)
