...
- **rayon global-pool init panic (FIXED).** Both engine dispatch loops (`engine.rs`) now run `par_iter` inside an explicitly-built pool via `with_worker_pool`: a cached default (`num_cpus`) pool, falling back to a single-thread `use_current_thread` pool (spawns zero new threads) under `RLIMIT_NPROC` / pids.max pressure (common in hardened CI / containers -- the self-hosted runner has hit pids limits). A constrained host degrades to sequential execution instead of the lazy-global-pool `.expect` panic + "alint crashed" banner; `install` preserves `par_iter`'s ordering, so results are identical.
