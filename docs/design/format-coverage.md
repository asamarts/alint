...
## 8. Known limitations (resource pressure) - tracked follow-ups

Surfaced by the pre-v0.16 adversarial audit (rounds 3-4). Every crafted-SMALL-file DoS is fixed: the HCL/YAML/XML depth guards, the HCL byte cap, the per-element XML attribute cap, and -- added in round 4 -- the YAML alias-EXPANSION guard (`yaml_depth::expansion_within_limit`; a single anchor referenced many times, or a `<<: *a` merge, which `serde_yaml_ng`'s own recursion/deref limits miss, ballooned a ~150 KB file into tens of millions of nodes). The items below each need an extreme input AND a resource-constrained host, and each is a GENERAL-engine concern (not specific to the format-coverage arc), so the fixes are deferred to a dedicated robustness pass rather than rushed into this hardening PR. Round 4 confirmed each is safely fixable and captured the approach:

1. **Parse-tree memory amplification.** `MAX_ANALYZE_BYTES` (256 MiB, `walker.rs`) bounds INPUT bytes, but a structured format expands into a `serde_json::Value` tree measured at ~16-19x the input. A near-cap file can peak at multiple GB of RSS, multiplied by the `par_iter` fan-out (default = num_cpus, so up to ~24 large files at once). Approach: a per-file structured-parse cap (a ~32-64 MiB byte cap, or a node cap analogous to the YAML alias budget -- a node count IS a memory bound) is a low-false-positive quick win for the single-file case (config/manifest files are tiny); the concurrency multiplier needs a concurrent-parse memory semaphore, which is the genuinely invasive part.
2. **rayon global-pool init can panic under thread-creation pressure.** Both engine dispatch loops (`engine.rs`) use rayon `par_iter`; if the OS refuses rayon's worker threads (`RLIMIT_NPROC` / pids.max, common in hardened CI / containers -- the alint-runner has hit pids limits) the lazy global-pool init panics (`rayon-core`), producing the "alint crashed" banner instead of a graceful error. Approach (measured zero steady-state overhead, ordering preserved): a cached scoped `rayon::ThreadPool` built with a fallback chain -- default multi-thread, then `num_threads(1).use_current_thread()` (spawns zero new threads) -- and `install()` the `par_iter` sections in it. Deferred not for feasibility but because it is a change to the engine's core dispatch that wants testing under real thread pressure + care around the cached `use_current_thread` pool (fine for the single-threaded CLI; needs thought for the multi-threaded LSP under pressure). (The arc's own analogue -- the HCL 512 MB parse thread failing to spawn -- IS already fixed: it returns a per-file parse-error instead of `.expect`-panicking.)
3. **XML multi-element attribute residual (bounded, non-quadratic).** The per-element attribute cap (now 256) restored linear-time parsing; a 256 MiB file of at-cap-width elements still costs a few seconds of CPU (linear in size, ~benign-XML speed at the 256 cap). A document-wide attribute budget or a parse watchdog would bound wall-clock further; not needed for the sharp DoS, which is closed.
