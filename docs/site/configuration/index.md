...
Membership is **directory-aware**: a declared file matches itself; a declared directory (a workspace member) matches every file under it, respecting component boundaries (`crates/a` does not match `crates/ab`). Declared paths are matched **literally**, not glob-expanded — a `members = ["crates/*"]` entry resolves to the literal `crates/*` and matches nothing (a silent no-op `expect_nonempty` cannot catch, since the set is non-empty); list explicit directories, or use `has_ancestor:` for a glob-defined workspace. The set is extracted once per run and cached, like `changed_since:`. A manifest that is absent or unreadable contributes nothing — the rule runs full-scope for `exclude`, matches nothing for `include`. A manifest **value** gates *which* files a rule sees, never *what* it decides about a file: content rules never read the manifest, extraction is pure-parse (no spawn, safe inside an `extends:`'d ruleset), and `alint explain <rule>` prints the resolved set.
