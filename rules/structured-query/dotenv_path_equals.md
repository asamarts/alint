...
### A .env.production set to the wrong NODE_ENV
...
```ansi
[2m--- .env.production ------------------------------------------------------------[0m
  [1m[31mx  error  [0m  [2mprod-node-env[0m
              value at path does not equal expected: expected "production", got
              "development"
...
### A .env.production correctly set to production
