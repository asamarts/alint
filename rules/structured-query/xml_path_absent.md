---
title: 'xml_path_absent'
description: 'Assert a JSONPath query over the document matches nothing; one file-level violation if present. alint xml_path_absent rule, structured query family.'
sidebar:
  order: 20
...
### A .csproj that opts into unsafe code
...
rules:
  - id: no-unsafe-blocks
    ...
    paths: "*.csproj"
    path: "$.Project.PropertyGroup.AllowUnsafeBlocks"
    level: error
    message: >-
      Project enables AllowUnsafeBlocks; unsafe code bypasses the memory
...
```ansi
[2m--- App.csproj -----------------------------------------------------------------[0m
  [1m[31mx  error  [0m  [2mno-unsafe-blocks[0m
              Project enables AllowUnsafeBlocks; unsafe code bypasses the memory
              guarantees the rest of the codebase relies on.
