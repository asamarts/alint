---
title: 'file_ends_with'
description: 'Byte-level prefix / suffix check. alint file_ends_with rule, content family.'
sidebar:
  order: 5
categories: ['content']
---

Byte-level prefix / suffix check. Works on any bytes (binary safe, unlike `file_header`).

Check-only: a fix would risk silently duplicating a near-matching prefix. Pair with `file_prepend` / `file_append` explicitly if you want auto-repair.

## Options

| Option | Type | Required | Default | Description |
|---|---|---|---|---|
| `suffix` | string | yes |  | Required suffix, matched byte-for-byte. |

Plus the common `paths`, `level`, `id`, and `when` fields. This table is generated from the JSON Schema; option types and defaults are authoritative.

## Example

### Generated files missing their sentinel line

The rule fires on this repository:

```text
build/
build/ok.generated.md
build/stripped.generated.md
build/wrong_tail.generated.md
```

```markdown title="build/ok.generated.md"
# hi

<!-- generated by alint -->
```

```markdown title="build/stripped.generated.md"
# hi
```

```markdown title="build/wrong_tail.generated.md"
# hi

<!-- generated by somebody else -->
```

With this `.alint.yml`:

```yaml
version: 1
rules:
  - id: gen-sentinel
    kind: file_ends_with
    paths: "build/**/*.generated.md"
    suffix: "<!-- generated by alint -->\n"
    level: error
```

`alint check` reports:

```ansi
[2m--- build/stripped.generated.md ------------------------------------------------[0m
  [1m[31mx  error  [0m  [2mgen-sentinel[0m
              file does not end with the required suffix

[2m--- build/wrong_tail.generated.md ----------------------------------------------[0m
  [1m[31mx  error  [0m  [2mgen-sentinel[0m
              file does not end with the required suffix

[2mSummary (2 violations):[0m
  [1m[31mx 2 errors[0m
  0 passing [2m*[0m 1 failing
```

### Every generated file ends with the sentinel

This repository is compliant:

```text
build/
build/a.generated.md
build/b.generated.md
```

```markdown title="build/a.generated.md"
# hi

<!-- generated by alint -->
```

```markdown title="build/b.generated.md"
# world

<!-- generated by alint -->
```

With this `.alint.yml`:

```yaml
version: 1
rules:
  - id: gen-sentinel
    kind: file_ends_with
    paths: "build/**/*.generated.md"
    suffix: "<!-- generated by alint -->\n"
    level: error
```

`alint check` reports:

```ansi
[1m[32mv All 1 rule(s) passed.[0m
```

## See also

- [`file_starts_with`](/docs/rules/content/file_starts_with/)
