...
**Headline finding:** angular's `goldens/public-api/<pkg>/` discipline
is **THE canonical example** of the v0.11+
`cross_language_implementation_complete` shape applied within a
single language — TypeScript source ↔ TypeScript API surface golden,
50 goldens locking the public surface of 13 of 16 packages — and
the live tree run surfaces **5 packages without an `index.api.md`
golden** (`benchpress`, `compiler`, `compiler-cli`,
`language-service`, `zone.js` — all internal/build-time packages
that should either ship a golden or appear on an explicit
allowlist), **5 packages without `public_api.ts`** (the canonical
API barrel file the goldens are extracted from), **6 source files
with non-canonical `@license` header** (3 with extra blank line,
1 with shebang preceding, 1 with leading UTF-8 BOM byte, 1 with
drifted block-comment style), and **2 packages with non-canonical
version field** (zone.js stuck at `0.16.1`, benchpress at
`0.4.0-PLACEHOLDER` instead of `0.0.0-PLACEHOLDER`) — drift no
existing tool catches because tslint sees source code, prettier
sees formatting, and `pnpm public-api:check` only runs against
goldens that already exist.
...
The `goldens/` directory is angular's **canonical mechanism** for
locking the public TypeScript API surface. ~50 `.api.md` files
across 13 of 16 packages, each generated by Microsoft's API
Extractor against the built bundles. The 3 packages without
goldens are `compiler`, `language-service`, `benchpress` — all
internal/build-time packages with no public TypeScript surface to
freeze.

This is the v0.11+ `cross_language_implementation_complete` shape
applied **within a single language** — TS source under
`packages/<name>/` ↔ TS API surface golden under
`goldens/public-api/<name>/index.api.md`. The starter config
expresses the forward direction (every package → golden) via
`for_each_dir`; the inverse direction (every golden → package)
needs `pair_inverse` (ruff's snapshot-freshness candidate).
...
- **`angular-source-license-header`** — every hand-edited
  ...
  wrappers prepend it unconditionally). **Surfaces 6 real drifts
  including a leading UTF-8 BOM that prettier doesn't catch.**
- **`angular-package-has-public-api-golden`** — every published
  `packages/<name>/` should have a corresponding
  `goldens/public-api/<name>/index.api.md`. **The forward
  direction of the v0.11+ `cross_language_implementation_complete`
  primitive applied within TypeScript** (TS source ↔ TS API
  surface). Surfaces 5 packages on the wrong side of the
  golden/no-golden split.
...
The `goldens/public-api/<pkg>/index.api.md` discipline is **THE
canonical example** of the v0.11+
`cross_language_implementation_complete` primitive applied within a
single language: every `packages/<name>/` (the source side) should
have a corresponding `goldens/public-api/<name>/index.api.md` (the
API-surface side), with explicit allowlisting for the
...
Where alint shines on angular specifically: the **per-package
manifest spot-checks** run against 16 `package.json` files in
single-digit milliseconds (sequential `node -e "require()"` calls
would be ~2-3 s of warm-cache startup). The **per-package golden
parity check** found 5 packages without goldens in milliseconds vs.
the human-review status quo. The **license-header consistency
check** caught 6 real drifts including a UTF-8 BOM byte that no
other tool in the pipeline looks at.
...
This case study is **the canonical "TS-monorepo + Bazel-built
mega-framework with strict per-package discipline" data point** for
the launch:

- angular is one of the most-watched JS framework repos on GitHub
  (~95k stars, ~25k forks). Naming it as a target gives alint
  instant credibility with the JS audience.
- The findings are real and actionable: 6 license-header drifts
  (including a UTF-8 BOM), 5 packages without API goldens, 5
  packages without `public_api.ts`, 2 with non-canonical version
  fields. **None of these would surface from running tslint /
  prettier / pnpm public-api:check** because tslint sees source
  semantics, prettier sees formatting, and `pnpm public-api:check`
  only diffs against goldens that already exist (a missing golden
  is silently a no-op).
...
- The `.ng-dev/` in-house CLI is a fascinating contrast to the
  ...
  invariants around its config surface (`format.mjs`,
  `commit-message.mjs`, `pull-request.mjs`,
  `google-sync-config.json`, `caretaker.mjs`) are exactly what
  alint expresses well.
- The contrast with microsoft/typescript (frozen-snapshot
  maintenance mode) and vercel/next.js (hybrid pnpm+Cargo
  dual-workspace) lands cleanly: angular sits in the **active
  TS-monorepo + Bazel** segment that no per-language linter
  catches end-to-end. Same shape is replicated in many enterprise
  TS-monorepos that use Bazel (Google's internal Angular build,
  Salesforce's Lightning Web Runtime, etc.).

Position it as the **fifth tile** on alint.org/examples (after
kubernetes, airflow, microsoft/typescript, vercel/next.js,
facebook/react), with the angle: "for active TS frameworks built on
Bazel with strict per-package discipline, alint is the layer that
sees the cross-language API parity contract that no per-language
linter does."
...
3. **`pair_inverse` rule kind** — covers the
   "every golden traces back to a package" inverse direction here,
   plus ruff's `cargo insta --unreferenced=reject` snapshot-freshness
   check. Demand: **2 case studies**.
