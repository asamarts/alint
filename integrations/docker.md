...
Tags published per release: the exact git tag (`:v0.14.1`), the bare semver (`:0.14.1`), the `<major>.<minor>` channel (`:0.14`), and `:latest`.
...
script:
  - docker run --rm -v "$PWD:/repo" ghcr.io/asamarts/alint:v0.14.1 check --format json
```
