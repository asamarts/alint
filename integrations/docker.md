...
Tags published per release: the exact git tag (`:v0.15.2`), the bare semver (`:0.15.2`), the `<major>.<minor>` channel (`:0.15`), and `:latest`.
...
script:
  - docker run --rm -v "$PWD:/repo" ghcr.io/asamarts/alint:v0.15.2 check --format json
```
