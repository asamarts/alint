...
Tags published per release: the exact git tag (`:v0.11.1`), the bare semver (`:0.11.1`), the `<major>.<minor>` channel (`:0.10`), and `:latest`.
...
script:
  - docker run --rm -v "$PWD:/repo" ghcr.io/asamarts/alint:v0.11.1 check --format json
```
