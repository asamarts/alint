...
Tags published per release: the exact git tag (`:v0.14.0`), the bare semver (`:0.14.0`), the `<major>.<minor>` channel (`:0.14`), and `:latest`.
...
script:
  - docker run --rm -v "$PWD:/repo" ghcr.io/asamarts/alint:v0.14.0 check --format json
```
